...
with open('../../days_inputs/day-11.txt', 'r') as f:
    ...
    for m in monkeys:
        common_divisor *= m.get_test_operand()
        # print(m)
    ...
    for r in range(1, nr_rounds+1):
        ...
        for monkey_nr, monkey in enumerate(monkeys):
            if r % 1000 == 0:
                print(f'\tmonkey {monkey_nr} inspections: {monkey.nr_inspections}')
            thrown_items = monkey.process(common_divisor)
    ...
    print(f'nr_inspections: {inspections}')
